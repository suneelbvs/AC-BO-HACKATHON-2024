...
with open('results.csv', 'r', encoding='utf-8') as input_file:
    ...
    for row in content:
        ...
        data[row[0]][row[1]][row[2]].append(list(map(int, row[3:])))
os.makedirs('output', exist_ok=True)
for dataset, dataset_values in data.items():
    for model, model_values in dataset_values.items():
        plt.figure()
        for acquisition_function, acquisition_function_values in model_values.items():
        ...
        plt.legend()
        plt.savefig(f'output/{dataset}_{model}.png')
        plt.close()
